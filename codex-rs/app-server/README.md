...
- `remoteControl/environment/updated` — notification emitted when the remote-control environment id exposed to remote clients changes. `environmentId` is a string when the app-server has a current enrollment and `null` when that enrollment is cleared or invalidated; newly initialized app-server clients receive the current non-null id.
