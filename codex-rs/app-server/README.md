...
- `plugin/uninstall` — uninstall a local plugin by `pluginId` by removing its cached files and clearing its user-level config entry, or uninstall a remote ChatGPT plugin by `remoteMarketplaceName` plus `pluginName` by forwarding the uninstall to the ChatGPT plugin backend (**under development; do not call from production clients yet**).
